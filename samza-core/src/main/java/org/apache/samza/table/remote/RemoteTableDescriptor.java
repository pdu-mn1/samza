...
public class RemoteTableDescriptor<K, V> extends BaseTableDescriptor<K, V, RemoteTableDescriptor<K, V>> {
  ...
  public TableSpec getTableSpec() {
    ...
    // Serialize and store record reader/writer with config
    tableSpecConfig.put(RemoteTable.READ_FN, serializeObject("record reader", readFn));

    if (writeFn != null) {
      tableSpecConfig.put(RemoteTable.WRITE_FN, serializeObject("record writer", writeFn));
    }
   ...
   * @param reader record reader instance
  ...
  /**
   * Use specified RecordWrite with remote table.
   * @param writer record writer instance
   * @return this table descriptor instance
   */
  ...
  protected void validate() {
    super.validate();
    Preconditions.checkNotNull(readFn, "Record reader is required.");
  }
}
